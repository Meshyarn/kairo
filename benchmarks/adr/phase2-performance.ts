...
async function measurePerformance(
    ...
    return {
        ...
        p95Ms,
        p99Ms,
        status: p95Ms < thresholdMs ? "PASS" : "FAIL",
...
async function benchmarkCallGraph(): Promise<BenchmarkResult> {
    ...
    return {
        ...
        status: "SKIP",
        threshold: 100,
        skipReason: "Requires dependency graph + symbol index setup."
    };
}
...
function printResults(results: BenchmarkResult[]): void {
    ...
    const totalRunnable = Math.max(0, results.length - skipped);
    console.log(`Results: ${passed}/${totalRunnable} benchmarks passed (skipped: ${skipped})`);
    console.log('='.repeat(80));

    if (passed < totalRunnable) {
        process.exit(1);
    }
}
