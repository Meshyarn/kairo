...
interface P2Metrics {
    ...
    vectorQueryLatencyP50?: number;
    vectorQueryLatencyP95?: number;
    vectorQueryLatencyP99?: number;
    ...
    // Storage
    embeddingsPackBytes: number;
    vectorIndexBytes: number;
    trigramIndexBytes: number;
...
async function runBenchmark() {
    ...
    const scenario: BenchmarkScenario = scenarioName
        ? readScenario(repoRoot, scenarioName)
        : {
              name: "default",
              rootPath: repoRoot,
              includeGlobs: ["src/**"],
    ...
    const scenarioRootPath = scenario.rootPath
        ? (path.isAbsolute(scenario.rootPath) ? scenario.rootPath : path.resolve(repoRoot, scenario.rootPath))
        : repoRoot;
    const effectiveRootPath = path.resolve(scenarioRootPath);
    PathManager.setRoot(effectiveRootPath);

    const nfs = new NodeFileSystem(effectiveRootPath);
    ...
    const largeFileRel = 'src/engine/Search.ts';
    const largeFileAbs = path.join(effectiveRootPath, largeFileRel);
    const content = fs.readFileSync(largeFileAbs, 'utf-8');
    await sg.generateSkeleton(largeFileRel, content); // warm parser/cache
    const s3 = performance.now();
    const skeleton = await sg.generateSkeleton(largeFileRel, content);
    ...
    const searchEngine = new SearchEngine(effectiveRootPath, nfs);
    ...
    const projectSymbolIndex = new SymbolIndex(effectiveRootPath, sg, []);
    const projectModuleResolver = new ModuleResolver(effectiveRootPath);
    const projectDependencyGraph = new DependencyGraph(effectiveRootPath, projectSymbolIndex, projectModuleResolver);
    await projectDependencyGraph.updateFileDependencies(largeFileAbs);
    const s7 = performance.now();
    await projectDependencyGraph.getDependencies(largeFileAbs, 'both');
    ...
    FileProfiler.analyzeMetadata(content, largeFileRel);
    ...
    const reportPath = path.resolve(repoRoot, `benchmarks/reports/full-report-${Date.now()}.md`);
    if (!fs.existsSync(path.dirname(reportPath))) fs.mkdirSync(path.dirname(reportPath), { recursive: true });

    const mem = process.memoryUsage();
    const smartContextDir = PathManager.resolve();
    const smartContextBytes = directorySizeBytes(smartContextDir);
    
    // P2 Storage breakdown
    const embeddingsPackDir = path.join(PathManager.getStorageDir(), "v1", "embeddings");
    const vectorIndexDir = PathManager.getVectorIndexDir();
    const trigramIndexPath = path.join(PathManager.getIndexDir(), "trigram-index.json");
    ...
    if (scenario.description) reportMd += `- description: ${scenario.description}\n`;
    reportMd += `- repoRoot: ${repoRoot}\n`;
    reportMd += `- effectiveRootPath: ${effectiveRootPath}\n`;
    ...
    reportMd += `\n### Storage Paths\n\n`;
    reportMd += `- storageDir: ${PathManager.getStorageDir()}\n`;
    reportMd += `- vectorIndexDir: ${vectorIndexDir}\n`;
    reportMd += `- trigramIndexPath: ${trigramIndexPath}\n`;
    reportMd += `- embeddingsPackDir: ${embeddingsPackDir}\n`;
