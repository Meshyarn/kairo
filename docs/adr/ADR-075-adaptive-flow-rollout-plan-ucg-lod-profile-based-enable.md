...
- `project_manage status` 응답에 rollout preset/userId hash/flag mode + gate 요약 노출.
...
- **진단 확인**: `manage({ command: "status" })`의 `rollout.cohort` / `rollout.adaptiveFlow.alertThresholds` / `rollout.adaptiveFlow.metrics`로 현재 설정을 확인한다.
- **경보 튜닝(기본값)**:
