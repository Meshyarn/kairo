# Configuration (Legacy: all env vars)
...
::: warning Legacy page
This is a monolithic “all env vars” page kept for older links and grep-ability. The canonical docs are the split reference above.

If you’re looking for `.kairo/config/*` files, start here: [Project config files](/reference/configuration/project-files)
:::
...
| `KAIRO_LOG_DIR` | Override log directory. | Default is `<KAIRO_DIR>/logs`. |
| `KAIRO_LOG_FILE` | Override log file path. | If set, all logs are written to this single file. |
...
## Project config files (recommended)

Project-local config lives under `.kairo/config/*` (in the target project root).

This legacy page is env-var focused; the canonical file-based config docs live in the split reference:
...
Tip: generate a starter skeleton with `manage({ command: "init", mode: "apply" })`.
