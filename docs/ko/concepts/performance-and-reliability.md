...
순수 풀 모델 실행 vs. 라우팅 전략 비교(단일 실행, 모델/가격 스냅샷에 종속):

| 지표 | Full Baseline (GPT-5.1 Codex) | Routed (Mini baseline + Mini+Kairo) | 변화 |
|------|--------|--------|--------|
| **Pass@1** | 100% | 100% | +0.0pp |
| **비용(8개 케이스 스위트 실행)** | $2.49 | $0.70 | -72.0% 💰 |
| **실행 시간(wall)** | 20.0분 | 25.6분 | +27.7% |
| **총 토큰** | 4.20M | 6.39M | +52.3% |

**해석:**
- 💰 pass@1이 비슷하더라도 라우팅으로 실지출을 크게 줄일 수 있습니다.
- ⏱️ wall-time/토큰 오버헤드는 구조화된 실행/검증에 따른 자연스러운 비용이며, 수용 가능 여부는 워크로드에 달려 있습니다.
- 📌 플래그/절차/다중 파일 일관성류 작업에서는 상위 모델 baseline에 근접하거나 경쟁적으로 보일 수 있지만, 결과는 레포/프롬프트에 따라 달라집니다.

#### 작동 방식

1. **절차적 검증**: Kairo는 검증 단계(드리프트 체크, 파일 검증, 구문 검증)를 강제하여 오류가 전파되기 전에 포착합니다.
2. **비용 효율성**: 소형 모델은 보통 토큰 단가가 훨씬 낮습니다. 많은 절차적 작업은 Mini baseline으로도 처리 가능하고, 일부는 구조화된 실행(Mini + Kairo)이나(필요 시) 상위 모델이 더 적합할 수 있습니다.
...
| 시나리오 | 권장사항 |
|----------|---|
| 비용에 민감한 작업 | ✅ 라우팅 사용 (기본 Mini baseline; 일부 케이스는 Mini+Kairo로 라우팅) |
| 안전이 중요한 작업 | ✅ 라우팅 사용 (검증 단계가 오류로부터 보호) |
| 높은 처리량의 에이전트 루프 | ✅ 라우팅 사용 (효율적으로 부하 분산) |
| 간단하고 명확한 작업 | ⚠️ 순수 Mini 고려 (라우팅이 불필요한 오버헤드일 수 있음) |
| 프로토타입/탐색 단계 | ⚠️ 순수 Full 고려 (더 빠른 반복), 이후 스케일 단계에서 라우팅 도입 |
