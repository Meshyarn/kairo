...
const createTempRoot = () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "kairo-mcp-inprocess-"));
  fs.mkdirSync(path.join(root, "src"), { recursive: true });
  fs.writeFileSync(path.join(root, "README.md"), "# MCP in-process mock\n", "utf-8");
...
const waitForNativeIndex = async (client, { timeoutMs = 30_000, intervalMs = 250 } = {}) => {
  ...
  throw new Error("Timed out waiting for native search index to become ready.");
};
...
async function main() {
  ...
  process.env.KAIRO_BASELINE_ENABLED = process.env.KAIRO_BASELINE_ENABLED ?? "on";
  process.env.KAIRO_EXPOSE_FILE_TOOLS = process.env.KAIRO_EXPOSE_FILE_TOOLS ?? "true";
  ...
  const client = new Client({ name: "kairo-mcp-inprocess-client", version: "0.1.0" });
