...
export class AttributeAccessIndex {
    ...
    private resolveVariableTypes(content: string, exportNames: Set<string>): Map<string, string> {
        ...
        const annotation = /\b([A-Za-z_]\w*)\s*:\s*([A-Za-z_][\w.]*)/g;
        const constructor = /\b([A-Za-z_]\w*)\s*=\s*([A-Za-z_][\w.]*)\s*\(/g;
        ...
        for (const match of content.matchAll(constructor)) {
            const name = match[1];
            const typeName = this.normalizeTypeName(match[2]);
            if (exportNames.has(typeName)) {
                result.set(name, typeName);
