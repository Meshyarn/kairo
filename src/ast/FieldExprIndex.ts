...
export class FieldExprIndex {
    ...
    private resolveVariableTypes(content: string, exportNames: Set<string>): Map<string, string> {
        ...
        const typedLet = /\blet\s+(?:mut\s+)?([A-Za-z_]\w*)\s*:\s*([A-Za-z_][\w:]*)/g;
        const structInit = /\blet\s+(?:mut\s+)?([A-Za-z_]\w*)\s*=\s*([A-Za-z_][\w:]*)\s*{/g;

        for (const match of content.matchAll(typedLet)) {
            const name = match[1];
        ...
        for (const match of content.matchAll(structInit)) {
            const name = match[1];
            const typeName = this.normalizeTypeName(match[2]);
            if (exportNames.has(typeName)) {
                result.set(name, typeName);
    ...
    private normalizeTypeName(typeName: string): string {
        ...
        const withoutModules = trimmed.split("::").pop() ?? trimmed;
