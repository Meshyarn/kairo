...
import { toRelativePath } from '../utils/PathHelpers.js';
...
export class TypeDependencyTracker {
    ...
    public async analyzeType(
        ...
        const relativePath = toRelativePath(this.rootPath, absPath);
    ...
    public invalidateFile(filePath: string) {
        const relativePath = toRelativePath(this.rootPath, filePath);
    ...
    public invalidateDirectory(dirPath: string) {
        const relative = toRelativePath(this.rootPath, dirPath);
        for (const key of Array.from(this.definitionCache.keys())) {
