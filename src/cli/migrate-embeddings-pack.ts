...
async function main(): Promise<void> {
    ...
    const rootPath = process.env.KAIRO_ROOT ?? process.cwd();
    PathManager.setRoot(rootPath);

    if (!process.env.KAIRO_EMBEDDING_PACK_FORMAT) {
        process.env.KAIRO_EMBEDDING_PACK_FORMAT = formatArg ?? "float32";
    } else if (formatArg) {
        process.env.KAIRO_EMBEDDING_PACK_FORMAT = formatArg;
    }
    ...
    if (provider === "disabled" || !model || model === "hash") {
        console.error("[embeddings-pack] Embeddings are disabled or unsupported; set KAIRO_EMBEDDING_MODEL.");
        process.exit(1);
    }
    ...
    if (!config.enabled) {
        console.error("[embeddings-pack] Pack is disabled; set KAIRO_EMBEDDING_PACK_FORMAT=float32|q8|both.");
        process.exit(1);
    }
