...
export class RolloutController {
    static applyFromEnv(): void {
        const presetKey = (process.env.KAIRO_ROLLOUT_MODE ?? process.env.KAIRO_ROLLOUT_PHASE ?? 'full').trim().toLowerCase();
        ...
        const canaryEnv = process.env.KAIRO_ROLLOUT_CANARY_USERS ?? process.env.KAIRO_CANARY_USERS;
        if (canaryEnv) {
            FeatureFlags.addCanaryUsers(canaryEnv.split(',').map(value => value.trim()).filter(Boolean));
        }

        const betaEnv = process.env.KAIRO_ROLLOUT_BETA_PERCENT ?? process.env.KAIRO_BETA_PERCENT;
        if (betaEnv) {
    ...
    private static shouldApplyPreset(): boolean {
        if (process.env.KAIRO_ROLLOUT_FORCE === 'true') {
            return true;
        }
