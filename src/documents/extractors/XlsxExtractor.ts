...
export async function extractXlsxAsText(absPath: string): Promise<XlsxExtractionResult> {
    ...
    try {
        ...
        const maxSheets = Number.parseInt(process.env.KAIRO_XLSX_MAX_SHEETS ?? `${DEFAULT_MAX_SHEETS}`, 10);
        const maxRows = Number.parseInt(process.env.KAIRO_XLSX_MAX_ROWS ?? `${DEFAULT_MAX_ROWS}`, 10);
        const maxCols = Number.parseInt(process.env.KAIRO_XLSX_MAX_COLS ?? `${DEFAULT_MAX_COLS}`, 10);
