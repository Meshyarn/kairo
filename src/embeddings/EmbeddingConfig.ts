...
export function resolveEmbeddingConfigFromEnv(): EmbeddingConfig {
    const providerRaw = process.env.KAIRO_EMBEDDING_PROVIDER;
    const provider = normalizeProvider(providerRaw);
    const normalize = process.env.KAIRO_EMBEDDING_NORMALIZE !== "false";
    const batchSize = parseOptionalInt(process.env.KAIRO_EMBEDDING_BATCH_SIZE);
    const timeoutMs = parseOptionalInt(process.env.KAIRO_EMBEDDING_TIMEOUT_MS) ?? DEFAULT_TIMEOUT_MS;
    const concurrency = parseOptionalInt(process.env.KAIRO_EMBEDDING_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const maxQueueSize = parseOptionalInt(process.env.KAIRO_EMBEDDING_MAX_QUEUE_SIZE);
    const modelCacheDir = process.env.KAIRO_MODEL_CACHE_DIR?.trim() || undefined;
    const modelDir = process.env.KAIRO_MODEL_DIR?.trim() || undefined;
