...
function logEmbeddingFailure(error: unknown): void {
    if (isTestEnv()) {
        return;
    }
    const message = typeof (error as any)?.message === "string"
        ...
        : "Unknown error";
    const stack = typeof (error as any)?.stack === "string"
        ? (error as any).stack
        : "";
    const stackSnippet = stack
        ? stack.split("\n").slice(0, 3).map((line: string) => line.trim()).join(" | ")
        : "";
    const details = stackSnippet ? ` | ${stackSnippet}` : "";
    console.warn(`[Embedding] Primary model failed; falling back to hash embeddings. Reason: ${message}${details}`);
}
