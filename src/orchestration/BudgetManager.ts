...
const readProfile = (fallback: BudgetProfile): BudgetProfile => {
  const raw = process.env.KAIRO_BUDGET_PROFILE;
...
export class BudgetManager {
  static create(inputs: BudgetInputs): ResourceBudget {
    const baseProfile = inputs.profile ?? (process.env.KAIRO_SAFE_MODE === 'false' ? 'balanced' : 'safe');
    ...
    const maxCandidatesEnv = envNumber('KAIRO_MAX_CANDIDATES');
    const maxFilesReadEnv = envNumber('KAIRO_MAX_FILES_READ');
    const maxBytesReadEnv = envNumber('KAIRO_MAX_BYTES_READ');
    const maxParseTimeMsEnv = envNumber('KAIRO_MAX_PARSE_MS');
