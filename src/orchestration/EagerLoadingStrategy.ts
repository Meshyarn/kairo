...
export class EagerLoadingStrategy {
  ...
  private async ensureCallGraph(
    ...
    const already = context.getFullHistory().some(step => step.tool === "relationship_analyze" && step.args?.mode === "calls");
    if (already) return;
    ...
    try {
      const output = await registry.execute("relationship_analyze", {
        target,
        mode: "calls",
        direction: "both",
        maxDepth
  ...
  private async ensureRelationships(
    ...
    const already = context.getFullHistory().some(step => step.tool === "relationship_analyze");
    if (already) return;
  ...
  private async ensureHotSpots(
    ...
    const already = context.getFullHistory().some(step => step.tool === "hotspot_detect");
    if (already) return;

    const started = Date.now();
    try {
      const output = await registry.execute("hotspot_detect", {});
      context.addStep({
        id: "eager_hotspots",
        tool: "hotspot_detect",
        args: {},
  ...
  private getHotSpotCount(context: OrchestrationContext): number {
    const step = context.getFullHistory().slice().reverse().find(s => s.tool === "hotspot_detect");
  ...
  private async ensureProfile(
    ...
    const already = context.getFullHistory().some(step => step.tool === "file_profile");
    if (already) return;
    ...
    const started = Date.now();
    try {
      const output = await registry.execute("file_profile", { filePath: target });
      context.addStep({
        id: "eager_profile",
        tool: "file_profile",
