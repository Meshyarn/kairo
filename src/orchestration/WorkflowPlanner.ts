  ...
  private buildFallbacks(intent: ParsedIntent): FallbackStrategy[] {
    if (intent.category === 'navigate' || intent.category === 'understand') {
      return [{
        ...
        steps: [
          { id: 'fallback_search', tool: 'project_search', params: { query: intent.originalIntent } }
        ]
      }];
    }
    return [];
  ...
  private buildEagerExpansions(intent: ParsedIntent): EagerExpansion[] {
    if (intent.category === 'navigate') {
      return [{
        ...
        steps: [
          { id: 'eager_profile', tool: 'file_profile', params: {}, inputFrom: 'search.output.results[0].path' }
        ]
      }];
    }
    if (intent.category === 'understand') {
      return [{
        ...
        steps: [{ id: 'eager_hotspots', tool: 'hotspot_detect', params: {}, parallel: true }]
      }];
    }
    return [];
  }
...
export class WorkflowPlanner {
  ...
  private getTemplateSteps(intent: ParsedIntent): WorkflowStep[] {
    ...
    switch (category) {
      case 'understand':
        return [
          { id: 'search', tool: 'project_search', params: { query: subject, type: 'symbol' } },
          { id: 'read_skeleton', tool: 'code_read', params: { view: 'skeleton' }, inputFrom: 'search.output.results[0].path' },
          { id: 'hotspots', tool: 'hotspot_detect', params: {}, parallel: true }
        ];

      case 'change':
        return [
          { id: 'search', tool: 'project_search', params: { query: subject, type: 'symbol' } },
          { id: 'read_full', tool: 'code_read', params: { view: 'full' }, inputFrom: 'search.output.results[0].path' },
          { id: 'impact', tool: 'impact_analyze', params: {}, inputFrom: 'search.output.results[0].path', parallel: true },
          { id: 'dry_run', tool: 'edit_transaction', params: { dryRun: true, edits: constraints.edits || [] }, inputFrom: 'search.output.results[0].path' }
        ];

      case 'navigate':
        return [
          { id: 'search', tool: 'project_search', params: { query: subject, limit: constraints.limit || 10 } },
          { id: 'profile', tool: 'file_profile', params: {}, inputFrom: 'search.output.results[0].path', condition: 'search.output.results.length === 1' }
        ];
      ...
      case 'read':
        return [
          { id: 'read', tool: 'code_read', params: { filePath: subject, view: constraints.depth === 'deep' ? 'full' : 'skeleton', lineRange: constraints.lineRange } }
        ];
      ...
      default:
        // 최소한의 검색 및 읽기 계획
        return [
          { id: 'search', tool: 'project_search', params: { query: subject } }
        ];
    }
  }
