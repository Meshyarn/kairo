...
const makeTempRoot = () => fs.mkdtempSync(path.join(os.tmpdir(), "kairo-emb-"));
...
describe("EmbeddingRepository", () => {
    ...
    it("supports multiple embeddings per chunk (provider/model composite key)", () => {
        if (!db) throw new Error("db missing");
        const chunkRepo = new DocumentChunkRepository(db);
        const embRepo = new EmbeddingRepository(db);
        ...
        const a = embRepo.getEmbedding("chunk-1", "local", "model-a");
        const b = embRepo.getEmbedding("chunk-1", "local", "model-b");
        expect(a).toBeTruthy();
        expect(b).toBeTruthy();
        expect(Array.from(a!.vector)).toEqual([1, 0, 0]);
        expect(Array.from(b!.vector)).toEqual([0, 1, 0]);
    });
});
