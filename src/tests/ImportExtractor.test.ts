...
describe('RegexBackend imports', () => {
  let extractor: RegexBackend;
  let testDir: string;
  const extractImports = async (filePath: string) => {
    const content = await fs.readFile(filePath, 'utf-8');
    return extractor.extractImports({ filePath, content });
  };
  
  beforeEach(async () => {
    testDir = path.join(__dirname, 'fixtures', 'import-test-' + Date.now());
    await fs.mkdir(testDir, { recursive: true });
    extractor = new RegexBackend(new ModuleResolver(testDir));
  });
