...
describe('Persistent Index', () => {
    ...
    beforeEach(async () => {
        const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'kairo-persistence-test-'));
    ...
    test('should restore state from persisted index', async () => {
        ...
        const persisted = await waitForPersistedIndex(indexManager);
        expect(persisted).not.toBeNull();
        expect(Object.keys(persisted!.files).length).toBeGreaterThan(0);

        const { indexer: idx2, symbolIndex: sym2, dependencyGraph: dep2 } = createIndexer(testProjectRoot);
        await idx2.start();
        await idx2.waitForInitialScan();

        const restoreCalls = (sym2.restoreFromCache as jest.Mock).mock.calls.length;
        const edgeCalls = (dep2.restoreEdges as jest.Mock).mock.calls.length;
        expect(restoreCalls + edgeCalls).toBeGreaterThan(0);
        await idx2.stop();
    });
...
async function waitForPersistedIndex(manager: ProjectIndexManager) {
    let last = await manager.loadPersistedIndex();
    for (let attempt = 0; attempt < 5; attempt += 1) {
        if (last && Object.keys(last.files).length > 0) {
            return last;
        }
        await sleep(200);
        last = await manager.loadPersistedIndex();
    }
    return last;
}
