...
beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kairo-doc-embed-"));
    PathManager.setRoot(tempDir);
    fs.mkdirSync(path.join(tempDir, "docs"), { recursive: true });
    fs.writeFileSync(path.join(tempDir, "docs", "notes.md"), "# Notes\n\n## Intro\nEager embedding test.");
    previousEnv = process.env.KAIRO_DOCS_EMBEDDINGS_EAGER;
    process.env.KAIRO_DOCS_EMBEDDINGS_EAGER = "true";
});

afterEach(() => {
    if (previousEnv === undefined) {
        delete process.env.KAIRO_DOCS_EMBEDDINGS_EAGER;
    } else {
        process.env.KAIRO_DOCS_EMBEDDINGS_EAGER = previousEnv;
    }
