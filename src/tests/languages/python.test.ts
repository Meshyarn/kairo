...
describe("Language support: Python", () => {
  ...
  it("extracts imports/symbols and skeleton", async () => {
    const content = loadFixture("python", "valid.py");
    const expected = loadExpected("python");
    const backend = await createBackend(manager);
    const doc = await manager.parseFile("sample.py", content);

    try {
      const languageId = manager.getLanguageId("sample.py");
      const imports = await backend.extractImports({ filePath: "sample.py", content, languageId, doc });
      const symbols = await backend.extractSymbols({ filePath: "sample.py", content, languageId, doc });
      const skeleton = await manager.generateUniversalSkeleton("sample.py", content);
  ...
  it("detects syntax errors", async () => {
    const content = loadFixture("python", "syntax-error.py");
    const issues = await validateSyntax("broken.py", content);
