...
describe("ExplorePillar", () => {
  ...
  it("reads full content for explicit paths when view=full", async () => {
    ...
    registry.register("code_read", async () => "console.log('ok');");
  ...
  it("filters to code_comment when include.docs=false and include.comments=true", async () => {
    ...
    const registry = new InternalToolRegistry();
    registry.register("document_search", async (args) => {
  ...
  it("blocks full reads when maxChars is exceeded", async () => {
    ...
    registry.register("code_read", async () => "a".repeat(50));
  ...
  it("degrades preview results when maxChars budget is exceeded", async () => {
    ...
    registry.register("code_read", async () => "x".repeat(20));
  ...
  it("skips document_search for symbol-like queries unless include is explicit", async () => {
    ...
    const context = new OrchestrationContext();
    const response = await pillar.execute(
      makeIntent({ query: "revokeMembershipByOrderNo stopUserMembership handleRevokeMembership" }) as any,
      context
    );
  ...
  it("runs document_search when include is explicitly provided", async () => {
    ...
    const context = new OrchestrationContext();
    const response = await pillar.execute(
      makeIntent({ query: "revokeMembershipByOrderNo", include: { docs: true } }) as any,
      context
    );
