...
describe('ADR-042-005 v2 Editor Integration', () => {
  ...
  let previousResolveTimeout: string | undefined;
  ...
  beforeEach(() => {
    ...
    process.env.KAIRO_EDITOR_V2_MODE = 'apply';
    previousResolveTimeout = process.env.KAIRO_EDITOR_RESOLVE_TIMEOUT_MS;
    process.env.KAIRO_EDITOR_RESOLVE_TIMEOUT_MS = '5000';
  });
  ...
  afterEach(() => {
    ...
    delete process.env.KAIRO_EDITOR_V2_MODE;
    if (previousResolveTimeout === undefined) {
      delete process.env.KAIRO_EDITOR_RESOLVE_TIMEOUT_MS;
    } else {
      process.env.KAIRO_EDITOR_RESOLVE_TIMEOUT_MS = previousResolveTimeout;
    }
  });
...
function test3() { return 42; }
  ...
  describe('Cost guardrails', () => {
    test('should block levenshtein on large file with short target', async () => {
      ...
      expect(duration).toBeLessThan(1200); // No expensive levenshtein
      expect(result.success).toBe(false);
