...
describe("SmartContextServer - code_read skeleton cache integration", () => {
    let server: SmartContextServer;
    let testRootDir: string;
    let targetFile: string;

    beforeEach(async () => {
        testRootDir = fs.mkdtempSync(path.join(os.tmpdir(), "kairo-skeleton-cache-"));
    ...
    test("skeleton results are cached in memory and on disk", async () => {
        ...
        const first = await (server as any).handleCallTool("code_read", { filePath: relativePath, view: "skeleton" });
        ...
        const second = await (server as any).handleCallTool("code_read", { filePath: relativePath, view: "skeleton" });
        const secondSkeleton = extractText(second);
        expect(secondSkeleton).toBe(firstSkeleton);
    });

    test("project_manage reindex clears skeleton caches and forces regeneration", async () => {
        const relativePath = path.relative(testRootDir, targetFile);
        
        await (server as any).handleCallTool("code_read", { filePath: relativePath, view: "skeleton" });
        ...
        await (server as any).handleCallTool("project_manage", { command: "reindex" });
        ...
        const responseAfterRebuild = await (server as any).handleCallTool("code_read", { filePath: relativePath, view: "skeleton" });
