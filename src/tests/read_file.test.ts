...
describe('SmartContextServer - file_read', () => {
...
`;
    ...
    beforeEach(async () => {
        testRootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kairo-test-'));
    ...
    afterEach(() => {
        ...
        const response = await (server as any).handleCallTool('file_read', args);
        ...
        expect(payload.meta.nextAction.tool).toBe('code_read');
        expect(payload.meta.nextAction.args.view).toBe('skeleton');
    });

    it('should truncate JSON-wrapped raw content when KAIRO_READ_FILE_MAX_BYTES is small', async () => {
        const prev = process.env.KAIRO_READ_FILE_MAX_BYTES;
        process.env.KAIRO_READ_FILE_MAX_BYTES = '32';
        try {
            const smallServer = new SmartContextServer(testRootDir);
            const args = { filePath: path.relative(testRootDir, fileA_Path), full: true };
            const response = await (smallServer as any).handleCallTool('file_read', args);
            const payload = JSON.parse(response.content[0].text);
        ...
        expect(payload.versionInfo?.contentHash).toBeDefined();
            ...
            if (prev === undefined) {
                delete process.env.KAIRO_READ_FILE_MAX_BYTES;
            } else {
                process.env.KAIRO_READ_FILE_MAX_BYTES = prev;
            }
        }
    });
