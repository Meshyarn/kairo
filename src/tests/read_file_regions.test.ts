...
describe('SmartContextServer - file_fragment_read', () => {
    let server: SmartContextServer;
    const testFilesDir = path.join(process.cwd(), 'src', 'tests', 'test_files', 'read_file_regions');
    const testFileName = 'read_test.txt';
    const relativeTestFilePath = path.join('src', 'tests', 'test_files', 'read_file_regions', testFileName);
    ...
    it('should extract lines with keyword matches', async () => {
        ...
        const response = await (server as any).handleCallTool('file_fragment_read', args);
