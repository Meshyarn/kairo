...
describe('SmartContextServer - file_edit', () => {
    let server: SmartContextServer;
    const testFilesDir = path.join(process.cwd(), 'src', 'tests', 'test_files', 'replace_in_file');
    const testFileName = 'replace_test.txt';
    let backupsDir: string;
    const relativeTestFilePath = path.join('src', 'tests', 'test_files', 'replace_in_file', testFileName);
    ...
    it('should enforce backup retention policy', async () => {
        ...
        await (server as any).handleCallTool('file_edit', args);
