...
beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kairo-pack-migrate-"));
    prevRoot = process.env.KAIRO_ROOT;
    prevFormat = process.env.KAIRO_EMBEDDING_PACK_FORMAT;
    prevRebuild = process.env.KAIRO_EMBEDDING_PACK_REBUILD;
    process.env.KAIRO_ROOT = tempDir;
});

afterEach(() => {
    if (prevRoot === undefined) delete process.env.KAIRO_ROOT;
    else process.env.KAIRO_ROOT = prevRoot;
    ...
    if (prevRebuild === undefined) delete process.env.KAIRO_EMBEDDING_PACK_REBUILD;
    else process.env.KAIRO_EMBEDDING_PACK_REBUILD = prevRebuild;
...
describe("Embedding pack migration", () => {
    it("auto migrates legacy embeddings into a pack", () => {
        ...
        process.env.KAIRO_EMBEDDING_PACK_REBUILD = "auto";
