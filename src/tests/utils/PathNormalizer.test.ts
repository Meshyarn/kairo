...
describe('PathNormalizer', () => {
    ...
    describe('normalizeWithSymlinks', () => {
        it('should normalize symlink targets to real paths', async () => {
            const target = path.join(testDir, 'file.ts');
            const link = path.join(testDir, 'file-link.ts');
            try {
            ...
            const symlinkNormalizer = new PathNormalizer(fs.realpathSync(testDir));
            const normalized = await symlinkNormalizer.normalizeWithSymlinks(link);
            expect(normalized).toBe('file.ts');
        });

        it('should fall back to normalize for missing files', async () => {
            const normalized = await normalizer.normalizeWithSymlinks('missing.ts');
            expect(normalized).toBe('missing.ts');
        });
    });

    describe('isSamePath', () => {
        it('should treat symlink and target as the same path', async () => {
            const target = path.join(testDir, 'file.ts');
            const link = path.join(testDir, 'file-alias.ts');
            try {
                fs.symlinkSync(target, link);
            } catch {
                return;
            ...
            const symlinkNormalizer = new PathNormalizer(fs.realpathSync(testDir));
            await expect(symlinkNormalizer.isSamePath(target, link)).resolves.toBe(true);
        });

        it('should return false for different paths', async () => {
            const pathA = path.join(testDir, 'file.ts');
            const pathB = path.join(testDir, 'src', 'index.ts');
            await expect(normalizer.isSamePath(pathA, pathB)).resolves.toBe(false);
        });
    });
