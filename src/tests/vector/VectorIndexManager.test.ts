...
beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kairo-vector-"));
...
afterEach(() => {
    if (prevStorage === undefined) {
        ...
        process.env.KAIRO_STORAGE_MODE = prevStorage;
    }
    if (prevIndex === undefined) {
        delete process.env.KAIRO_VECTOR_INDEX;
    } else {
        process.env.KAIRO_VECTOR_INDEX = prevIndex;
    }
    if (prevRebuild === undefined) {
        delete process.env.KAIRO_VECTOR_INDEX_REBUILD;
    } else {
        process.env.KAIRO_VECTOR_INDEX_REBUILD = prevRebuild;
    }
...
describe("VectorIndexManager", () => {
    it("falls back to bruteforce when auto+manual and no index exists", async () => {
        process.env.KAIRO_VECTOR_INDEX = "auto";
