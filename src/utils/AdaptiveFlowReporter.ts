...
export class AdaptiveFlowReporter {
    ...
    private resolveInterval(): number | null {
        const raw = this.options.exportIntervalMs ?? Number(process.env.KAIRO_METRICS_INTERVAL_MS ?? 60000);
    ...
    private pickThreshold<K extends keyof AlertThresholds>(key: K, fallback: number): number {
        ...
        const envKeyMap: Record<keyof AlertThresholds, string> = {
            topologySuccessRate: 'KAIRO_TOPOLOGY_SUCCESS_MIN',
            ucgMemoryMb: 'KAIRO_UCG_MEMORY_MAX_MB',
            l3PromotionRatio: 'KAIRO_L3_PROMOTION_RATIO_MAX'
        };
    ...
    private shouldScheduleDailyExport(): boolean {
        const raw = process.env.KAIRO_METRICS_DAILY_EXPORT;
