...
export class PathNormalizer {
    ...
    constructor(rootDir: string, logger?: any) {
        this.rootDir = PathHelpers.normalize(rootDir);
        this.logger = logger;
    }
    ...
    normalize(inputPath: string): string {
        ...
        if (PathHelpers.isAbsolute(inputPath)) {
            absolutePath = PathHelpers.normalize(inputPath);
        } else {
            // 상대경로는 루트 기준으로 해석
            absolutePath = PathHelpers.resolve(this.rootDir, inputPath);
        }
        ...
        let relativePath = PathHelpers.relative(resolvedRoot, absolutePath);
