...
export function resolveVectorIndexConfigFromEnv(): VectorIndexConfig {
    const mode = normalizeMode(process.env.KAIRO_VECTOR_INDEX) ?? DEFAULT_CONFIG.mode;
    const rebuild = normalizeRebuild(process.env.KAIRO_VECTOR_INDEX_REBUILD) ?? DEFAULT_CONFIG.rebuild;
    const maxPoints = parseNumber(process.env.KAIRO_VECTOR_INDEX_MAX_POINTS, DEFAULT_CONFIG.maxPoints);
    const m = parseNumber(process.env.KAIRO_VECTOR_INDEX_M, DEFAULT_CONFIG.m);
    const efConstruction = parseNumber(process.env.KAIRO_VECTOR_INDEX_EF_CONSTRUCTION, DEFAULT_CONFIG.efConstruction);
    const efSearch = parseNumber(process.env.KAIRO_VECTOR_INDEX_EF_SEARCH, DEFAULT_CONFIG.efSearch);
    const shards = normalizeShards(process.env.KAIRO_VECTOR_INDEX_SHARDS) ?? DEFAULT_CONFIG.shards;
